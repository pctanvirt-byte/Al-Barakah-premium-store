...
interface AdminDashboardProps {
  ...
  onToggleEnableCoupons?: (enabled: boolean) => void;
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const handleRequestMasterKeyOtp = async () => {
    ...
    try {
      ...
      try {
        ...
        data = { error: 'সার্ভারের সাথে সংযোগ স্থাপন করা সম্ভব হয়নি। দয়া করে কিছুক্ষণ পর আবার চেষ্টা করুন।' };
      }
      if (!response.ok) {
  ...
  const handleSubmitMasterKeyChange = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        data = { error: 'সার্ভারের সাথে সংযোগ স্থাপন করা সম্ভব হয়নি।' };
      }
      if (!response.ok) {
  ...
  // Coupons State
  const [coupons, setCoupons] = useState<CouponItem[]>(activeCoupons);
  const [newCouponCode, setNewCouponCode] = useState('');
  const [newCouponDiscount, setNewCouponDiscount] = useState(10);
  const [newCouponMin, setNewCouponMin] = useState(1000);

  useEffect(() => {
    if (Array.isArray(activeCoupons)) {
  ...
  const handleAddCoupon = (e: React.FormEvent) => {
    ...
    if (coupons.some((c) => c.code === codeClean)) {
      alert('এই কুপন কোডটি ইতিমধ্যে তালিকায় বিদ্যমান রয়েছে!');
      return;
    }
    ...
    setNewCouponCode('');
    showToast('✅ নতুন কুপন ডাটাবেজে যুক্ত হয়েছে!');
  };
  ...
  const handleDeleteCoupon = (id: string) => {
    ...
    showToast('🗑️ কুপন মুছে ফেলা হয়েছে');
  };
    ...
    <div className="min-h-screen bg-[#F4F6F8] flex text-stone-800 antialiased font-sans relative">
      ...
      {productToDelete && (
        <div className="fixed inset-0 z-50 bg-stone-950/75 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in zoom-in-95 duration-200">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-5 border border-stone-200">
            ...
            <div className="p-3.5 bg-stone-50 rounded-2xl border border-stone-200 flex items-center gap-3">
              <img
                src={productToDelete.image || (productToDelete.images && productToDelete.images[0]) || 'https://images.unsplash.com/photo-1578849278619-e73505e9610f?w=800&auto=format&fit=crop&q=80'}
                alt={productToDelete.name}
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-[11px] text-emerald-800 font-bold mt-1">
                  ৳{Math.round(productToDelete.price * rate).toLocaleString()} • {productToDelete.category}
                </div>
              </div>
            </div>

            <div className="p-3 bg-rose-50 rounded-xl border border-rose-100 text-xs text-rose-800 leading-relaxed font-medium">
              ⚠️ আপনি কি নিশ্চিতভাবে <strong>"{productToDelete.name}"</strong> প্রোডাক্টটি মুছে ফেলতে চান? এটি ফায়ারবেস ক্লাউড ডাটাবেজ থেকে স্থায়ীভাবে রিমুভ হয়ে যাবে এবং রিফ্রেশ দিলেও আর আসবে না।
            </div>
      ...
      {isMasterKeyModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-sm animate-in fade-in">
          <div className="relative w-full max-w-lg max-h-[90vh] flex flex-col bg-stone-900 border border-[#D4AF37]/50 rounded-3xl shadow-2xl text-stone-100 overflow-hidden">
            {/* Pinned Header */}
            <div className="shrink-0 flex items-center justify-between p-4 sm:p-5 border-b border-stone-800 bg-stone-900 z-10">
              <div className="flex items-center gap-2.5">
                <div className="w-9 h-9 rounded-xl bg-[#D4AF37]/20 border border-[#D4AF37]/40 flex items-center justify-center">
                  <KeyRound className="w-5 h-5 text-[#D4AF37]" />
                </div>
                <div>
                  <h3 className="text-sm sm:text-base font-bold text-white font-serif">মাস্টার সিকিউরিটি কি পরিবর্তন</h3>
                  <p className="text-[10px] sm:text-[11px] text-stone-400">Super Admin (pctanvirt@gmail.com) OTP Verified</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSubmitMasterKeyChange} className="flex-1 flex flex-col min-h-0">
              <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4">
                ...
                <div className="p-3.5 sm:p-4 rounded-2xl bg-stone-950/60 border border-stone-800 space-y-3">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    ...
                    <button
                      ...
                      className="px-3.5 py-1.5 rounded-xl bg-emerald-800 hover:bg-emerald-700 text-white text-xs font-bold transition-colors disabled:opacity-50 cursor-pointer flex items-center justify-center gap-1.5 shrink-0"
                    >
                      {isRequestingMkOtp ? (
                        <>
                          <RefreshCw className="w-3.5 h-3.5 animate-spin" />
              ...
              {/* Pinned & Always Visible Submit & Cancel Footer */}
              <div className="shrink-0 p-3 sm:p-4 bg-stone-950/95 border-t border-stone-800 flex items-center justify-end gap-3 z-10">
                <button
                  type="button"
